...
from mlse_diff_go_runtime import GO_ABI_RUNTIME_IR, go_abi_extra_runtime_ir
...
def go_abi_map_string_string_setup(name: str) -> tuple[list[str], str, str, list[str], str]:
    same_name = f"%{name}_same"
    return [
        f"  %{name}_old_map = call ptr @__mlse_map_string_string_new()",
        f"  %{name}_new_map = call ptr @__mlse_map_string_string_new()",
    ], f"%{name}_old_map", f"%{name}_new_map", [
        f"  {same_name} = call i1 @__mlse_map_string_string_equal(ptr %{name}_old_map, ptr %{name}_new_map)"
    ], same_name
...
def go_abi_input_pair_setup(param: dict[str, Any]) -> tuple[list[str], str, str, list[str], str | None]:
    name = sanitize_symbol(param["name"])
    if param["type"] == "map_string_string":
        return go_abi_map_string_string_setup(name)
    lines, value = go_abi_input_setup(param)
    return lines, value, value, [], None
...
def go_abi_symbolic_global_names(param: dict[str, Any]) -> list[str]:
    ...
    model_type = param["type"]
    length = int(param.get("length", 1))
    if model_type == "map_string_string":
        return []
    if model_type == "slice_string":
...
def build_go_abi_klee_harness(metadata: dict[str, Any], old_symbol: str, new_symbol: str) -> str:
    ...
    old_arg_values: list[str] = []
    new_arg_values: list[str] = []
    post_compare_blocks: list[tuple[list[str], str]] = []
    state_lines, state_globals = go_abi_symbolic_state_setup(model)
    setup_lines.extend(state_lines)
    for param in params:
        lines, old_value, new_value, post_compare, post_compare_value = go_abi_input_pair_setup(param)
        setup_lines.extend(lines)
        old_arg_values.append(old_value)
        new_arg_values.append(new_value)
        if post_compare_value:
            post_compare_blocks.append((post_compare, post_compare_value))
    old_args = ", ".join(
        f"{go_abi_llvm_type(param['type'])} {value}" for param, value in zip(params, old_arg_values)
    )
    new_args = ", ".join(
        f"{go_abi_llvm_type(param['type'])} {value}" for param, value in zip(params, new_arg_values)
    )
    compare_lines = go_abi_compare_lines(return_type)
    final_same = "%same"
    for index, (lines, value) in enumerate(post_compare_blocks):
        compare_lines.extend(lines)
        combined = f"%same_with_effect_{index}"
        compare_lines.append(f"  {combined} = and i1 {final_same}, {value}")
        final_same = combined
    ...
    compare_text = "\n".join(compare_lines)
    if return_type == "void":
        call_text = f"""  call {ret_decl} @{old_symbol}({old_args})
  call {ret_decl} @{new_symbol}({new_args})"""
    else:
        call_text = f"""  %old_result = call {ret_decl} @{old_symbol}({old_args})
  %new_result = call {ret_decl} @{new_symbol}({new_args})"""
...
{call_text}
{compare_text}
  br i1 {final_same}, label %ok, label %mismatch
