...
- `motus-mod4` 到 `motus-mod39`：从 Motus `pkg/analysis/ssa/smtcmp/testdata/` 批量抽出的函数级 old/new fixture；其中 `mod4`、`mod5`、`mod6`、`mod7`、`mod9` 当前会进入最小 `[]int` KLEE harness，`mod11` / `mod16` / `mod17` 会进入 Go LLVM ABI harness，其余 case 作为 expected blocker 进入 readiness matrix
...
这条命令会为当前 repo-owned 样例生成 same-input KLEE harness，重命名并链接 old/new bitcode，再检查等价样例无 KLEE `.err` 文件、非等价样例能通过 `assert.err` 产生 counterexample。当前 KLEE harness 有两类 Go ABI 支持：`slice_i64` 覆盖 `[]int` 的最小 ABI；`go_llvm` 覆盖 bounded symbolic `string`、`[]string`、逻辑输出比较，以及测试所需的 `runtime.add.string`、`runtime.any.box.*`、`runtime.fmt.Sprintf` 的 `%s` 子集、`runtime.makeslice`、`runtime.growslice`、`runtime.newobject`、`runtime.composite.map` 和 `runtime.panic.index`。数字格式化、context、map 语义和外部 client 仍然保持 unsupported / inconclusive。
