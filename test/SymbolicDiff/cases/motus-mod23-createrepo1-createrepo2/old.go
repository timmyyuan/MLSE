...
import (
	...
	"example.com/smtcmpmod23/logs"
	"example.com/smtcmpmod23/metrics"
	"example.com/smtcmpmod23/util_http"
	"example.com/smtcmpmod23/utils"
)
...
func F(ctx context.Context, gitRepo, jwt, departmentId string, grootNodeId uint64, owners []string, repoInfo *CreateRepoResp) (int, error) {
...
func CreateRepo2(ctx context.Context, gitRepo, jwt, departmentId string, grootNodeId uint64, owners []string, repoInfo *CreateRepoResp) (int, error) {
	authToken, err := GetAuthToken(ctx, jwt)
	if err != nil {
		return -1, err
	}
